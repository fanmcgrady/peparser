...
class Structure:
    ...
    def dump(self, indentation=0):
        ...
        dump = []
        #  <p class="clr">Dos头(IMAGE_DOS_HEADER)<a name="dos"></a></p>  
        dump.append('<br>')     
        dump.append('<p class="clr">[{0}]<a name="{0}"></a></p>'.format(self.name))
        dump.append('<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb"> <tr class="head"> <td width="126">Offset</td><td width="126">RVA</td><td>Description</td><td width="355">Value</td></tr>')
        ...
        for keys in self.__keys__:
            for key in keys:
                ...
                if( keycount == 0 ):
                    dump.append('<tr><td>0x%X</td> <td>0x%X</td> <td><span class="special">%s</span></td> <td><span class="special">%s</span></td></tr>' % (
                    self.__field_offsets__[key] + self.__file_offset__,
                    self.__field_offsets__[key], key + ':', val_str))
                else:
                    dump.append('<tr><td>0x%X</td> <td>0x%X</td> <td>%s</td> <td>%s</td></tr>' % (
                    self.__field_offsets__[key] + self.__file_offset__,
                    self.__field_offsets__[key], key + ':', val_str))
                keycount += 1
        dump.append('</table>')
...
class PE:
    ...
    def __parse__(self, fname, data, fast_load):
        ...
        if not self.DOS_HEADER or self.DOS_HEADER.e_magic != IMAGE_DOS_SIGNATURE:
            raise PEFormatError('DOS_HEADER not found.')
    ...
    def getsectionname(self):
        ...
        for section in self.sections:
            arr.append(section.Name)
        return arr
    
    def getSectionFlags(self):
        section_flags = self.retrieve_flags(SECTION_CHARACTERISTICS, 'IMAGE_SCN_')
        res = []
        for section in self.sections:
            flags = []
    ...
    def getCodeSectionEntropy(self):
        entropy = ''
        section_flags = self.retrieve_flags(SECTION_CHARACTERISTICS, 'IMAGE_SCN_')
        codesection = ''
        for section in self.sections:
        ...
        entropy = str(get_list_entropy(codesection))
        return entropy
    ...
    def getDataSectionEntropy(self):
        ...
        for section in self.sections:
            ...
            for flag in section_flags:
                if getattr(section, flag[0]):
                    flags.append(flag[0])
            if 'IMAGE_SCN_MEM_EXECUTE' not in flags:
                datasection += section.data
        entropy = str(get_list_entropy(datasection))
        return entropy
    
    def getFileEntropy(self):
        return get_list_entropy(self.__data__)
    
    def getHeaderEntropy(self):
        entropy = ''
        ...
        optional_header_offset =    \
            nt_headers_offset+4+self.FILE_HEADER.sizeof()
        datasection = ''
        datasection += self.__data__[nt_headers_offset:]+self.__data__[nt_headers_offset+4:]+self.__data__[optional_header_offset:]
        entropy = str(get_list_entropy(datasection))
        return entropy
    
    def getIATEntryNumber(self):
        count = 0
        if hasattr(self, 'DIRECTORY_ENTRY_IMPORT'):
            importDescDatas = self.DIRECTORY_ENTRY_IMPORT
            countList = []
            for module in importDescDatas:
                countList.append( len( self.get_import_table(module.struct.FirstThunk) ) )
    #             count += len( self.get_import_table(module.struct.FirstThunk )
            for c in countList:
                count += c
        return count
#         return int('00068950',16)
    
    def get_allsections_Entropy(self):
        s = []
        for section in self.sections:
            s.append(section.Name.strip('\x00')+":"+str(section.get_entropy()))
        return s
...
#         dump.add_newline()
        ...
        for section in self.sections:
            ...
            dump.add('<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb"><tr><td><font color="#E19A24" >Flags: ')
            flags = []
            for flag in section_flags:
                if getattr(section, flag[0]):
                    flags.append(flag[0])
            dump.add_line(' , '.join(flags))
            dump.add("</font></td></tr></table>")
...
#                                     ': ' + var_entry.entry.values()[0])
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_EXPORT'):
            ...
            dump.add_newline()
            dump.add('<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb" style="table-layout:fixed;">')
            dump.add_line(u'<tr class="head"><td>%s</td><td>%s</td><td>%s</td>' % ('Ordinal', 'RVA', 'Name'))
            for export in self.DIRECTORY_ENTRY_EXPORT.symbols:
                if export.name:
                    name = export.name
                    dump.add(u'<tr class="head"><td>%d</td><td> 0x%Xh</td> <td>%s</td>' % (
                        export.ordinal, export.address, name))
                if export.forwarder:
                    dump.add_line(u' <tr class="head"><td>forwarder: {0}</td>'.format(
                        export.forwarder))
                else:
                    dump.add_newline()
            dump.add('</table>')
            ...
            for module in self.DIRECTORY_ENTRY_IMPORT:
                ...
                if not module.imports:
                    dump.add('  Name -> {0}'.format(
                        self.get_string_at_rva(module.struct.Name)))
                    dump.add_newline()
                dump.add_newline()
                dump.add('<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb" style="table-layout:fixed;">')
                dump.add('<tr class="head"><td>name</td><td>' + self.get_string_at_rva(module.struct.Name) + '</td><td></td><td></td></tr>')
                dump.add('<tr class="head"><td>import function name</td><td>import function name</td><td>import function name</td><td>import function name</td></tr>')
                dump.add('<tr>')
                mmm = 0
                for symbol in module.imports:
                    if symbol.import_by_ordinal is True:
                        if symbol.name is not None:
                            dump.add('<td style="word-break:break-all;word-wrap:break-word;">{2}</td>'.format(
                            ...
                            mmm = mmm + 1
                            dump.add('<td style="word-break:break-all;word-wrap:break-word;">{1}</td>'.format(
...
#                     dump.add_line('Type: 0x%x(Unknown)' % dbg.struct.Type)
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_BASERELOC'):
            ...
            for base_reloc in self.DIRECTORY_ENTRY_BASERELOC:
                ...
                dump.add('<br>')
                dump.add('<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb" style="table-layout:fixed;">')
                dump.add('<tr class="head"><td>Index</td><td>RVA</td><td>RELOCATION_TYPE</td><td>Index</td><td>RVA</td><td>RELOCATION_TYPE</td><td>Index</td><td>RVA</td><td>RELOCATION_TYPE</td></tr>')
                dump.add('<tr>')
                for reloc in base_reloc.entries:
                    try:
                        dump.add('<td><font color="#E19A24" >%s</font></td><td>%Xh</td> <td>%s</td>' % (mm+1,
                            reloc.rva, RELOCATION_TYPE[reloc.type][16:]), 4)
                        mm = mm + 1
                        if mm % 3 == 0:
                                dump.add('</tr><tr>')
                    except KeyError:
                        dump.add('<td><font color="#E19A24" >%s</font></td><td>0x%X</td> <td>0x%x(Unknown)</td>' % (mm+1,
...
if __name__ == '__main__':
    codesection = PE(sys.argv[1]).getIATEntryNumber()
    print codesection
    
#     codesection = PE(sys.argv[1]).DIRECTORY_ENTRY_IMPORT
#     for m in codesection:
#         print hex( m.struct.FirstThunk )
#     rva = PE(sys.argv[1]).OPTIONAL_HEADER.DATA_DIRECTORY[12].VirtualAddress 
#     data = PE(sys.argv[1]).get_data( rva, Structure(PE(sys.argv[1]).__IMAGE_IMPORT_DESCRIPTOR_format__).sizeof() )
#     import_desc =  PE(sys.argv[1]).__unpack_data__(
#                 PE(sys.argv[1]).__IMAGE_IMPORT_DESCRIPTOR_format__,
#                 data, file_offset = PE(sys.argv[1]).get_offset_from_rva(rva) )
#     print len( PE(sys.argv[1]).get_import_table(import_desc.OriginalFirstThunk) )
